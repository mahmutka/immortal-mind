...
from cognitio.attention import MultiHeadAttention
from cognitio.character import CharacterManager, CognitiveState, PersonalityVector
...
from cognitio.temporal import TemporalDensityTracker
...
class CognitioEngine:
            ...
            try:
                ...
                if _boost > 0:
                    ...
                    logger.debug(
                        ...
                        self.predictive.last_error, _boost, emotional_tone,
                    )
            except Exception as _e:
    ...
    def _run_checkpoint(self) -> dict:
        ...
        if self.narrative.should_reflect(self.state.total_interactions):
            try:
                ...
                self.narrative.take_snapshot(
                    _pv_dict, _ep_dict, self.state.total_interactions
                )
    ...
    def _add_memory_from_pending(self, pending: dict) -> tuple[bool, Optional[str]]:
        ...
        if _raw_valence not in _valid_valences:
            _raw_valence = next(
                (v for v in _raw_valence.split("|") if v.strip() in _valid_valences),
                "neutral"
            )
        ...
        if _raw_mt not in _valid_mts:
            _raw_mt = next(
                (t for t in _raw_mt.split("|") if t.strip() in _valid_mts),
                "episodic"
            )
        memory = MemoryRecord(
            content=content,
    ...
    def _load_state(self, filepath: str) -> None:
        ...
        try:
            ...
            if "relational" in data:
                from cognitio.character import RelationalProfile
    ...
    def soft_reset(self) -> dict:
        ...
        logger.info(
            "Soft reset complete: cleared=%d, genesis=%d", cleared, genesis_count
        )
    ...
    def full_delete(self) -> dict:
        ...
        for f in (
            ...
            + glob.glob(os.path.join(self.data_dir, "*.db"))
        ):
            try:
                os.remove(f)
