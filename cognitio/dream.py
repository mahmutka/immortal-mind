...
class DreamCycle:
    ...
    def _emotional_regulation(
        self, engine: "CognitioEngine"
    ) -> tuple[int, list[str]]:
        """
        ...
        for memory in memories:
            ...
            if new_val != old_val:
                ...
                regulated += 1
                log.append(
                    f"Emotional softening ({old_val:.2f}→{new_val:.2f}): "
                    f"{memory.content[:60]}..."
                )
    ...
    def _find_insights(
        ...
        for i, mem_a in enumerate(sample):
            ...
            for mem_b in sample[i + 1:]:
                ...
                sim = engine.embedder.cosine_similarity(
    ...
    def should_dream(self, sleep_duration_seconds: Optional[float]) -> bool:
        ...
        if self.last_dream_at is not None:
            session_age = (
                datetime.now(timezone.utc) - self.last_dream_at
            ).total_seconds()
